...
class TrainManager(object):
	...
	def train(self):
		...
		for epoch in range(epochs):
			...
			logger.log_value('loss', loss.item(), step=epoch)
			logger.log_value('val_acc', val_acc, step=epoch)
			logger.log_value('lr', self.optimizer.param_groups[0]['lr'], step=epoch)
			
			if val_acc > best_acc:
				best_acc = val_acc
				self.save(epoch, name='{}_{}_best.pth.tar'.format(self.name, trial_id))
			# print current epoch loss and accuracy for each epoch
			print('epoch: {} - loss: {:.4f} - val_acc: {:.4f}'.format(epoch, loss.item(), val_acc))
