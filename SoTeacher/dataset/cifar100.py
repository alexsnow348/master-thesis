...
def get_data_folder():
    ...
    # else:
    data_folder = './cifar-100-python_2/'
...
def get_cifar100_dataloaders(batch_size=128, num_workers=8, is_instance=False):
    ...
    assert(not hasattr(train_loader, 'size'))
    assert(not hasattr(train_loader, 'n_classes'))
    train_loader.size = len(train_set)
    train_loader.n_classes = len(train_set.classes)
